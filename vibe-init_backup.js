...
async function main() {
    ...
    const vibeKey = await select({
        message: 'Choose your Vibe (Philosophy):',
        options: Object.entries(VIBES).map(([key, val]) => ({ value: key, label: val.name, hint: val.desc })),
    });
    ...
    if (!stackKey) {
        stackKey = await select({
            ...
            options: Object.entries(STACKS).map(([key, val]) => ({ value: key, label: val.name, hint: val.desc })),
        });
    ...
    Restart your editor to load the new context.
    `));
